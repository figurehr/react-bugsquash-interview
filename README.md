# react-bugsquash-interview

This question is inspired from a real open-source react component.

Everything from the Installation section onwards is from the original repository.

Please use Yarn, as we've seen issue with package hoisting with npm.
...
You can run a react app running the library from the docs/ folder. Like so:
```bash
$ cd docs/
$ yarn install
$ yarn start
